...
from modules.logger import log_request, log_auth_event, get_logger
...
async def register(user: UserCreate, db: Session = Depends(get_db)):
    ...
    if db_user:
        if db_user.username == user.username:
            log_auth_event("register", user.username, False, reason="username_exists")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Username already registered"
            )
        else:
            log_auth_event("register", user.username, False, reason="email_exists")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
async def login(user_credentials: UserLogin, db: Session = Depends(get_db)):
    ...
    if not user:
        log_auth_event("login", user_credentials.username, False, reason="invalid_credentials")
        raise HTTPException(
...
async def refresh_token(refresh_request: RefreshTokenRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if not user or not user.is_active:
            log_auth_event("refresh_token", username, False, reason="user_not_found_or_inactive")
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
        ...
        log_auth_event("refresh_token", username, True)
